...
double g_WRITE_SPEED = 0.0;
double g_WRITE_PERCENT = 0.0;
const char *g_WRITE_UNIT = "";
...
static void measure(int fd, struct flow *fw)
{
	...
	if (fw->progress) {
        ...
        g_WRITE_SPEED = inst_speed;
        g_WRITE_PERCENT = percent;
        g_WRITE_UNIT = unit;
		assert(fw->erase > 0);
	...
	struct flow *fw)
{
	...
	while (remaining > 0) {
        ...
        //printf("DEBUG=%d\n", g_FLAG_shouldTerminate);
        /*if (g_FLAG_shouldTerminate!=0) {
            //fine=0;
            remaining = 0;
            break;
        }*/
...
double f3write_fill_fs(const char *path, int start_at, int progress)
{
	...
	/* Writing speed. */
	if (fw.measurements > 0) {
		ret_speed = get_avg_speed(&fw);
		const char *unit = adjust_unit(&ret_speed);
...
double f3write_getCurrentSpeed(void) {
    return(g_WRITE_SPEED);
}

double f3write_getCurrentPercent(void)
{
    return(g_WRITE_PERCENT);
}
